...
C   equation domain decomposition.  The initial temperature is computed
C   to be
...
C   time-stepping, an array containing two domains is used; these
C   domains
...
C   time step, workers processes must exchange border data with
C   neighbors,
C   because a grid point's current temperature depends upon it's
C   previous
C   time step value plus the values of the neighboring grid points.
C   Upon
C   completion of all time steps, the worker processes return their
C   results
C   to the master process.
C 
C   Two data files are produced: an initial data set and a final data
C   set.
C   An X graphic of these two states displays after all calculations
C   have
C   completed.
C 
C AUTHOR: Blaise Barney - adapted from D. Turner's serial C version.
C Converted
...
C     Distribute work to workers.  Must first figure out how many cols
C     to
...
C     Now receive my offset, cols, neighbors and grid partition from
C     master 
      source = MASTER
      msgtype = BEGIN
      call MPI_RECV( offset, 1, MPI_INTEGER, source,
...
C     neighbors.  If I have the first or last grid col, then I only need
C     to 
