...
!  Reference:
!
!    Geoffrey Fox, Mark Johnson, Gregory Lyzenga, Steve Otto, John
!    Salmon, 
!    David Walker,
...
!
!    Local, integer ( kind = 4 ) N_LOCAL, the number of points visible
!    to 
!    this process.
!
...
!
!    Output, real ( kind = 8 ) U1_LOCAL(N_LOCAL), the portion of the
!    solution
...
!
!  The global array of N_GLOBAL points must be divided up among the
!  processes.
...
!
  do i = 1, nsteps
    ...
    t = dt * real ( i, kind = 8 )
! 
!  For the first time step, we need to use the initial derivative
!  information.
!
    if ( i == 1 ) then
...
!
!  After the first time step, we can use the previous two solution
!  estimates.
!
    else
...
!    Input, real ( kind = 8 ) DT, the size of the time step.
!
!    Input, real ( kind = 8 ) U_LOCAL(N_LOCAL), the final solution
!    estimate 
